...
namespace CompassNavigatorPro {
    ...
    public partial class CompassPro : MonoBehaviour {
        ...
        float currentMiniMapStateZoomLevel {
            get {
                //return _miniMapZoomState ? _miniMapFullScreenZoomLevel : _miniMapZoomLevel;
        ...
        Vector2 GetMiniMapScreenPos(Vector3 poiPosition) {
            ...
            float zoomLevel = currentMiniMapStateZoomLevel;
            viewportPos.x = (viewportPos.x - 0.5f) / zoomLevel + 0.5f;
            viewportPos.y = (viewportPos.y - 0.5f) / zoomLevel + 0.5f;
            float aspectRatio = miniMapUIRootRT.rect.size.x / miniMapUIRootRT.rect.size.y;
            viewportPos.y = (viewportPos.y - 0.5f) * aspectRatio + 0.5f;

            viewportPos += _miniMapIconPositionShift;
            return viewportPos;
        }
